<?php

declare(strict_types=1);

namespace Integration\Scenario;

use Helpers\NullBasicClientGenerator;
use Helpers\StickerTestClientGenerator;
use Helpers\TraceConverter\TraceConverterJsonToText;
use JsonException;
use PHPUnit\Framework\TestCase;
use TelegramOSINT\Client\InfoObtainingClient\Models\StickerSetModel;
use TelegramOSINT\Exception\TGException;
use TelegramOSINT\Scenario\FeaturedStickerSetScenario;
use TelegramOSINT\Scenario\GetStickerSetScenario;
use TelegramOSINT\Scenario\Models\StickerSetId;

class GetFeaturedStickersTest extends TestCase
{
    private const DEFAULT_AUTHKEY = '79803100357:::7b2263726561746564223a313533323434373530342c226170695f6964223a362c2264635f6964223a322c2264635f6970223a223134392e3135342e3136372e3530222c2264635f706f7274223a3434337d';
    private const TRACE_PATH = '/traces/get-stickers.json';
    private const TIMEOUT = 0.15;

    /**
     * @throws JsonException
     * @throws TGException
     * @noinspection PhpUnusedParameterInspection
     */
    public function testGetFeatured(): void
    {
        $file = TraceConverterJsonToText::fromFile(__DIR__.self::TRACE_PATH);
        $baseGenerator = new NullBasicClientGenerator(json_decode($file, true, 512, JSON_THROW_ON_ERROR));
        $generator = new StickerTestClientGenerator($baseGenerator, self::DEFAULT_AUTHKEY);

        $count = 0;
        $echo = static function (StickerSetModel $set) use (&$count) {
            $count++;
        };
        $fn = static function (StickerSetModel $set) use ($echo, $generator) {
            $scenario = new GetStickerSetScenario(
                StickerSetId::of($set),
                $echo,
                false,
                $generator
            );
            $scenario->startActions(false);
        };
        $scenario = new FeaturedStickerSetScenario($fn, $generator);
        $scenario->setTimeout(self::TIMEOUT);
        $scenario->startActions();
        self::assertEquals(17, $count);
    }
}
