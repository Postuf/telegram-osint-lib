<?php

declare(strict_types=1);

namespace Integration\Scenario;

use Helpers\NullBasicClientGenerator;
use Helpers\TestClientGenerator;
use PHPUnit\Framework\TestCase;
use TelegramOSINT\Exception\TGException;
use TelegramOSINT\Scenario\ReusableClientGenerator;
use TelegramOSINT\Scenario\SearchUserScenario;

class SearchMembersScenarioTest extends TestCase
{
    private const DEFAULT_AUTHKEY = '79803100357:::7b2263726561746564223a313533323434373530342c226170695f6964223a362c2264635f6964223a322c2264635f6970223a223134392e3135342e3136372e3530222c2264635f706f7274223a3434337d';
    private const TRACE_PATH = '/traces/group-members-search.txt';
    private const TRACE_PATH_NOT_FOUND = '/traces/group-members-notfound.txt';
    private const TIMEOUT = 0.15;

    /**
     * Test members search positive (user found)
     *
     * @throws TGException
     */
    public function test_members_search(): void
    {
        $file = file_get_contents(__DIR__.self::TRACE_PATH);
        $baseGenerator = new NullBasicClientGenerator(json_decode($file, true));
        $count = 0;
        $handler = function () use (&$count) {
            $count++;
        };
        $client = new SearchUserScenario(
            new ReusableClientGenerator(new TestClientGenerator($baseGenerator, self::DEFAULT_AUTHKEY)),
            ['asfaefegw'],
            'ntlvikhhjofnekge',
            $handler
        );
        $client->setTimeout(self::TIMEOUT);
        $client->startActions();
        $this->assertEquals(1, $count);
    }

    /**
     * Test members search negative (user not found)
     *
     * @throws TGException
     */
    public function test_members_search_negative(): void
    {
        $file = file_get_contents(__DIR__.self::TRACE_PATH_NOT_FOUND);
        $baseGenerator = new NullBasicClientGenerator(json_decode($file, true));
        $count = 0;
        $handler = function () use (&$count) {
            $count++;
        };
        $client = new SearchUserScenario(
            new ReusableClientGenerator(new TestClientGenerator($baseGenerator, self::DEFAULT_AUTHKEY)),
            ['asfaefegw'],
            'ntlvikhhjofnekge',
            $handler
        );
        $client->setTimeout(self::TIMEOUT);
        $client->startActions();
        $this->assertEquals(0, $count);
    }
}
